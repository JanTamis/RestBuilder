...
public class RequestBodySerializer : DiagnosticAnalyzer
{
	...
	private void AnalyzeMethod(SymbolAnalysisContext context)
	{
		...
		foreach (var attribute in method.GetAttributes())
		{
			...
			if (attribute.AttributeClass is { Name: "RequestBodySerializerAttribute", ContainingNamespace: { Name: "RestBuilder" } })
			{
		...
		if (!method.HasAttribute("RequestBodySerializerAttribute"))
		{
			return;
		}
		...
		var parentHasBodies = method.ContainingType.GetMembers()
			...
			.Any(m => m.IsPartial() && m.Parameters.Any(a => a.HasAttribute("BodyAttribute")));
